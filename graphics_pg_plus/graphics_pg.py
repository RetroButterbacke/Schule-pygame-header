...
class Text:
    ...
    original: pg.Surface = field(init=False, compare=False)
    loaded: bool = field(init=False, compare=False, default=False)
    surfs: Dict[Tuple[vec2 | None, int | None, int | None], pg.Surface | None] = field(compare=False, init=False)
    
    def __post_init__(self) -> None:
        object.__setattr__(self, 'surfs', {})
    
    def __load_textPart__(self, start: vec2 = vec2(0, 0), width: int = None, height: int = None) -> None:
        #### TODO
        pass
        # width = width or self.text_surface.get_width()
        # height = height or self.text_surface.get_height()
        
        # if not (0 <= start.x < self.text_surface.get_width() and 0 < start.y < self.text_surface.get_height()):
        #     raise IndexError("Starting position is out of range")

        # result = pg.Surface((width, height), pg.SRCALPHA)
        
        # source_rect = pg.Rect(start.x, start.y, width, height)
        # result.blit(self.text_surface, (0, 0), source_rect)
        
        # object.__setattr__(self, 'text_surface', result)
        
    def reset(self) -> None:
        self.text_surface = self.original
    ...
    def load(self, width: int, height: int, color: rgb | rgba) -> None:
        if not self.loaded:
            ...
            object.__setattr__(self, 'original', self.text_surface)
            object.__setattr__(self, 'loaded', True)
            
    def convert(self, scaled: bool, starting_pos: int | None = None, fontSize: int | None = None) -> None:
        #### TODO
        pass
        # if not self.surfs.get((starting_point, part_width, part_height), None):
        #     if not starting_point:
        #         self.__load_textPart__(width, height)
        #     else:
        #         if part_width and part_height:
        #             self.__load_textPart__(starting_point, part_width, part_height)
        #             object.__setattr__(self, 'text_surface', self.getScaled(width, height))
        #         else:
        #             raise TypeError("Missing arguments part_width, part_height")
        #     self.loaded[(starting_point, part_width, part_height)] = self.text_surface
        # else:
        #     object.__setattr__(self, 'text_surface', self.loaded[(starting_point, part_width, part_height)])
        #     object.__setattr__(self, 'text_surface', self.getScaled(width, height))
...
class Button:
    def __init__(self, width: int, height: int, pos: vec2, label: None | str, runOnClick: Callable) -> None:
        ...
        self.range = pg.Rect(*topleft._get(), width, height)
        self.width = width
        self.height = height
        self.pos = pos
        self.topx = topleft.x
...
class InputField:
    def __init__(self, pos: vec2, width: int, height: int, allow_next_line: bool = True) -> None:
        self.text = ""
        self.allow_next_line = allow_next_line
        
        self.topleft = pos.convert(width, height, "tl")
        self.bottomleft = pos.convert(width, height, "bl")
        self.pos = pos
        self.width = width
        self.height = height
        self.range = pg.Rect(*self.topleft._get(), width, height)
        ...
        self.mark_begin = -1
        self.mark_end = -1
        self.mark_box = pg.Rect(*self.topleft._get(), 0, height)
        
        self.last_click_time = 0
        self.click_counter = 0
        self.CLICK_TIME_THRESHOLD = 200
        self.last_time = 0
        
    def _set_focused(self, pos: vec2) -> None:
        self.in_focus = self.range.collidepoint(*pos._get())
        self.cursor = True
        
    def _update(self, text: str | None, input: InputListener, mouse_pos: vec2) -> None:
        if self.in_focus:
            current_time = pg.time.get_ticks()    
            if text and len(text) == 1:
                if (input.isKeyPressed("LEFT SHIFT") or input.isKeyPressed("RIGHT SHIFT")) and not (input.isKeyPressed("LEFT CTRL") or input.isKeyPressed("RIGHT CTRL")):
                    self.text += text
                elif (input.isKeyPressed("LEFT CTRL") or input.isKeyPressed("RIGHT CTRL")):
                    if text == "A":
                        self.mark_begin = 0
                        self.mark_end = len(text) - 1
                    elif text == "C":
                        if self.mark_begin > -1 and self.mark_end > -1:
                            clip = ""
                            for i in range(self.mark_begin, self.mark_end):
                                clip += self.text[i]
                            pc.copy(clip.strip())
                        else:
                            pc.copy(self.text)
            ...
            if self.last_click_time == 0 and input.wasButtonPressed("LEFT"):
                self.last_click_time = current_time           
            if current_time - self.last_click_time > self.CLICK_TIME_THRESHOLD and self.click_counter > 0:
    ...
    def draw(self, window: 'Window', font_size: int = 12, fontStyle: str | None = None, fontColor: rgb = rgb(0, 0, 0), cursor_color: rgb | rgba = rgb(0, 0, 0), outlined: bool = False, outline_depth: int = 0, background_color: rgb | rgba = rgba(255, 255, 255, 255), outlineColor: rgb | rgba = rgba(255, 255, 255, 255), border_radius: int = 0, background_texture: None | str = None, outline_texture: None | str = None, background_scaled: bool = True, outline_scaled: bool = True, background_transparency: int = 255, outline_transparency: int = 255, rotation: int = 0) -> None:
        self.isDrawn = True
        ...
        window.drawRect(self.pos, self.width, self.height, background_color, background_texture, scaled=background_scaled, rotation=rotation, transparency=background_transparency, border_radius=border_radius)
        if outlined:
            window.drawRect(self.pos, self.width, self.height, outlineColor, outline_texture, scaled=outline_scaled, rotation=rotation, transparency=outline_transparency, lineDepth=outline_depth, border_radius=border_radius)
        if self.in_focus:
            if self.cursor:
                window.drawLine(vec2(self.topleft.x + (outline_depth + 5), self.topleft.y  + (outline_depth + 5)), vec2(self.bottomleft.x  + (outline_depth + 5), self.bottomleft.y  - (outline_depth + 5)), cursor_color, 2)
        #window.drawText(self.pos, )        
...
class Window:
    ...
    def drawText(self, pos: vec2, width: int, height: int, text: str, color: rgb | rgba = rgba(0, 0, 0, 255), fontStyle: None | str = None, *, scaled: bool = False, starting_point: vec2 = vec2(0, 0), part_width: int | None = None, part_height: int | None = None, rotation: int = 0, transparency: int = 255) -> None:
        global initialized_texts
        topleft: vec2 = pos.convert(width, height, "tl")
        text_instance: Text = Text(text, fontStyle)
        if not text_instance in initialized_texts:
        ...
        text_instance.rotate(rotation)
        if scaled:
            text_instance.convert(width, height, starting_point, part_width, part_height)
            self.screen.blit(text_instance.get(), (topleft.x + ((width // 2) // 4) // 2, topleft.y + ((height // 2) // 4) // 2))
        else:
    ...
    def addInputField(self, field: InputField) -> None:
    ...
    def startGameLoop(self, gameLoop: Callable, escape_sequence: Tuple[str, ...] | str, framerate: int, input: None | InputListener = None) -> None:
        ...
        if len(self.input_fields) > 0 and not input:
            print("Cannot use input fields when no input listener is defined")
            sys.exit()
        ...
        while self.running:
                    ...
                    if event.type == pg.KEYDOWN:
                        if event.key in keys:
                            ...
                            for field in self.input_fields:
                                field._update(keys[event.key], input, self.getMousePos( ))
                    escape = False
            ...
            for field in self.input_fields:
                field._update(None, input, self.getMousePos())    
            ...
            if input:
                if input.wasButtonPressed("LEFT"):
                    ...
                    for field in self.input_fields:
                        if field.isDrawn:
                            field._set_focused(self.getMousePos())
